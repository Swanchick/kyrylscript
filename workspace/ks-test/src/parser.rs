...
#[test]
fn sort_functions() -> KsResult<()> {
    let test_statements = vec![
        Statement::Function {
            name: String::from("prior_function1"),
            public: false,
            return_type: DataType::Void(None),
            parameters: Vec::new(),
            body: Vec::new(),
        },
        Statement::Function {
            name: String::from("prior_function2"),
            public: false,
            return_type: DataType::Void(None),
            parameters: Vec::new(),
            body: Vec::new(),
        ...
        Statement::VariableDeclaration {
            ...
            public: false,
            data_type: Some(DataType::Int),
            value: Some(Expression::IntegerLiteral(20)),
        },
    ];

    let driver = KsDriver::new("parser/sort_functions.ks");
    let statements = driver.parser()?;

    assert_eq!(test_statements, statements);
